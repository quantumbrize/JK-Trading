...
<script>
    ...
    // Store the stock value of the selected size
        $(element).data('stock', stock); // Store stock value in the 'data-stock' attribute
        ...
        console.log('Selected Size:', $(element).text()); // Display selected size in the console
        console.log('Stock for selected size:', stock);  // Log the stock value
    }
    ...
    $(document).ready(function () {
        ...
        //             html5=`<span style="font-size:19px;"><b>Description</b></span>  ${resp.data.description}`
                    
        //                 $('#product_img_indecator').html(html1);
        //                 $('#product_main_image').html(html2);
        //                 $('#size_chart_link').html(html4);
        //                 $('#keyword_div').html(html3);
        //                 $('#product_description').html(html5);
        //                 $('#size_chart_link_pc').html(html4);
            ...
            success: function (resp) {
                ...
                if (resp.status) {
                    ...
                    if(resp.data.product_img != ""){
                        ...
                        var html_main_img = `<img class="varient_img"
                                        src="<?= base_url() ?>/public/uploads/product_images/${resp.data.product_img[0].src}"
                                        onclick="view_image('${encodeURIComponent(JSON.stringify(resp.data.product_img))}', '${var_id = ""}','${encodeURIComponent(JSON.stringify(resp.data.product_sizes))}', 'main')" alt="Image 1"> `
                        $('#all_varient_img').append(html_main_img)
                        } else {
                        html1 += `<li
                                    data-target="#carouselExampleFade"
                                    data-slide-to="1"
                        ...
                        var html_main_img = `<img class="varient_img"
                                        src="<?=base_url()?>public/assets/ztImages/demo_img.jpg"
                                        onclick="view_image('', '${var_id = ""}', '${encodeURIComponent(JSON.stringify(resp.data.product_sizes))}', 'main')" alt="Image 1"> `
                        $('#all_varient_img').append(html_main_img)
                    }
                    html3= `<h4 class="product-keyword" id="product_keyword"> <b>${resp.data.tags}</b></h4>`
                    html4=`<a href="javascript:void(0);" onclick="showSizeChart('${resp.data.uid}')">See Size Chart</a>`
    ...
    // Get the selected size element (the one with the 'active' class)
        ...
        if (!activeSize) {
            ...
            stopOnFocus: true,
            style: {
                background: 'darkred',  // Change background color to red for error
            },
        }).showToast();
        return;
        }

        // Get the stock value of the selected size
        const selectedStock = $(activeSize).data('stock');  // Get the stock value stored in the data attribute

        // Get the product quantity
        let product_quantity = parseInt($('#quantity').text(), 10);

        // Check if the stock is sufficient for the requested quantity
        if (product_quantity <= selectedStock) {
            // If the stock is sufficient, proceed with adding to cart
                    ...
                    if (resp.status) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                size: $(activeSize).data('size'),  // Pass the size of the selected size
                                stock: selectedStock  // Pass the selected stock value
                            },
                            success: function (resp) {
                                Toastify({
                                    text: resp.message.toUpperCase(),
                        ...
                        Toastify({
                            text: 'Please login to add to cart'.toUpperCase(),
                            duration: 3000,
                            position: "center",
                            stopOnFocus: true,
                            style: {
                ...
                if (resp.status) {
                    ...
                    $.each(resp.data, function (index, products) {
                        ...
                        total_quantity = 0; // Reset total_quantity for each product
                                ...
                                for (let i = 0; i < products.product_sizes.length; i++) {
                                    total_quantity += parseInt(products.product_sizes[i].stocks, 10); // Add stocks to total_quantity
                                }
                ...
                if (resp.status) {
                    ...
                    $.each(resp.data, function (index, variation) {
                        ...
                        html_img += `<img class="varient_img"
                                        ...
                                        onclick="view_image('/public/uploads/variant_images/${variation.product_img[0].src}', '${variation.uid}','${encodeURIComponent(JSON.stringify(variation.product_sizes))}', 'varient')" alt="Image 1"> `
                    })
                    // $('#product_size').html(html_size)
    ...
    function view_image(product_img, var_id, sizes, type) {
        var html = ``;
        variation_id = var_id;
        ...
        $('#product_img_indecator').html(html);
        $('#product_main_image').html(html2);
